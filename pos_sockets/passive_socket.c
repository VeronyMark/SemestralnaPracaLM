...
_Bool passive_socket_start_listening(struct passive_socket* self, short port) {
    ...
    //kontrola ci uz nahodou v nejakom vlakne nerobim listening
    //listening iba v 1 vlakne
...
//accept v 1 vlakne
//cakam kym sa pripoji ak sa pripoji ukoncenie a "vypadnem odtial" ->return true

//ak chcem 2 stroje 2 krat v cykle volam wait
// n klientov n krat
//ak passive vrati aktivny socket mozem robit komunikaciu
_Bool passive_socket_wait_for_client(struct passive_socket* self, struct active_socket* client_sock) {
    ...
    struct timeval tv;
    tv.tv_usec = 0;
    //accept je blokujuce -> neviem ho nasilu ukoncit zastavit
    //komplikovane iba si zobrat z toho to ze z blokujucich urobime neblokujuve
    while (passive_socket_is_listening(self)) {
        ...
        if (FD_ISSET(self->socket_descriptor, &sockets)) {
           //dolezite ze na tomto riadku volam accept
           //ak sa ku mne niekto pripoji tento riadok prebehne uspesne
           //do client_socket info o klientovi
